...
def group_season(data, geogroup, target):
    """aggregates target column by considering weights for daytype
    and number of days in season.
    """
    if isinstance(target, list):
        targets = target
    else:
...
def get_order(col, param):
    ...
    return list(parameter[col].values)
    ...
    for item in df.index.unique():
        ...
        if isinstance(subset, pd.Series):
            basedf = get_base_dataframe(
                typecols, type_, demand_sector).reset_index()
            ...
            basedf = get_base_dataframe(
                typecols_, type_, demand_sector).reset_index()
            if exact:
                del basedf['dummy']
                ...
                v = subset.isin(basedf.to_dict(orient='list')).all().all()
